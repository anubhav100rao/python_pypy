...
def test_md5():
    ...
    assert md5(b"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789").hex(
    ) == 'd174ab98d277d9f5a5611c2c9f419d9f'
    assert md5(b"12345678901234567890123456789012345678901234567890123456789012345678901234567890").hex(
...
def types_of_attacks():
    ...
    # hours to a few days to compute
    def chosen_prefix_attack(prefix0: bytes, prefix1: bytes, suffix: bytes):
        bz0 = bytes(...)
        bz1 = bytes(...)
        assert md5(prefix0 + bz0 + suffix) == md5(prefix1 + bz1 + suffix)

    # none publicly known, 2**123 theoretical time
    def second_preimage_attack(bz0: bytes):
        bz1 = bytes(...)
